...
class AlumnadoDAOImpl : AlumnadoDAO {
    ...
    override fun seleccionarAlumnado(): List<Alumnado> {
        ...
        val query =
            "SELECT id,nombre, edad, fecha_nacimiento, matriculado, DATE_FORMAT(fecha_nacimiento, '%d/%m/%Y') AS fecha_nacimiento_string FROM ALUMNO"
        ...
        while (rs?.next() == true) {
            val alu = Alumnado(
                ...
                rs.getString("fecha_nacimiento_string")
            )
            lista.add(alu)
        }
        st?.close()
